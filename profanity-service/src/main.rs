...
fn main() {
    ...
    // Ensure ONNX Runtime DLL can be found
...
fn start_video_monitoring(
    ...
    // print environment information to help diagnose path issues
    if let Ok(cwd) = std::env::current_dir() {
        println!("📂 Current working directory: {}", cwd.display());
    }
    if let Ok(envp) = std::env::var("NSFW_MODEL_PATH") {
        println!("🌍 NSFW_MODEL_PATH env var: {}", envp);
    }

    println!("📹 Attempting to load NSFW model name: {}", NSFW_MODEL_PATH);
    let nsfw = match NsfwDetector::new(NSFW_MODEL_PATH) {
        Ok(n) => {
            println!("✅ NSFW detector ready (NudeNet 320n)");
            Arc::new(parking_lot::Mutex::new(n))
...
/// Automatically locate and configure ONNX Runtime DLL path if not already set.
///
/// This function checks several locations for `onnxruntime.dll` and sets the
/// `ORT_DYLIB_PATH` environment variable if needed. The search order is:
///
/// 1. Respect existing `ORT_DYLIB_PATH` env var (highest priority).
/// 2. Check the directory containing the current executable.
/// 3. Check the current working directory.
/// 4. Check `profanity-service/` relative to CWD (for development runs).
/// 5. Check `onnxruntime-win-x64-1.24.2/lib/` relative to CWD (bundled with repo).
///
/// If found, the env var is set for the current process. If not found, a warning
/// is printed but execution continues (NSFW detection will fail at load time with
/// a clearer error message).
fn setup_ort_dylib_path() {
    use std::env;
    use std::path::{Path, PathBuf};

    // If already set, respect it and return
    if env::var("ORT_DYLIB_PATH").is_ok() {
        if let Ok(path) = env::var("ORT_DYLIB_PATH") {
            println!("🔧 ORT_DYLIB_PATH already set: {}", path);
        }
        return;
    }
    ...
    if let Ok(cwd) = env::current_dir() {
        ...
        // 3. profanity-service subdirectory
        candidates.push(cwd.join("profanity-service").join("onnxruntime.dll"));
        // 4. bundled ONNX Runtime lib folder
        candidates.push(
            cwd.join("profanity-service")
    ...
    for candidate in &candidates {
        if candidate.exists() {
            let dll_path = candidate.to_string_lossy().into_owned();
            env::set_var("ORT_DYLIB_PATH", &dll_path);
            println!("🔧 ORT_DYLIB_PATH set to: {}", dll_path);
            return;
        }
    }

    // If we get here, DLL was not found
    eprintln!(
        "⚠️ ONNX Runtime DLL (onnxruntime.dll) not found in checked locations:"
    );
    for c in &candidates {
        eprintln!("   - {}", c.display());
    }
    eprintln!("   NSFW detection will fail. Either:");
    eprintln!("   1. Copy onnxruntime.dll next to the executable, OR");
    eprintln!("   2. Set ORT_DYLIB_PATH environment variable to the full DLL path.");
}
